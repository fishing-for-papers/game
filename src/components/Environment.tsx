...
import { contourConfig, contourOutermostColor, environmentLandColor } from '../config/contourConfig'
import { MARGIN, OUTLINE_PADDING } from '../config/layoutConstants'
...
function Environment() {
  ...
  const { width, height, contentWidth, contentHeight, xScale, yScale } = useCoordinateSystem()

  const { outlinePath, squigglyOutlinePath } = useMemo(() => {
    if (papers.length === 0 || contentWidth <= 0 || contentHeight <= 0) {
      return { outlinePath: '', scaledPoints: [] }
    }

    const scaledPoints = papers.map((p) => ({
      x: xScale(p.x),
      y: yScale(p.y),
      cluster: p.cluster,
    }))
    ...
    // Close the path for simplification (add first point at end if needed)
    ...
    // Create variable width path for squiggly outline
    const variableWidthConfig = {
      baseWidth: 5.5,
      widthVariation: 0.8,
      noiseScale: 12,
      samplesPerPixel: 0.15,
      minSamples: 20,
      maxSamples: 200,
    }
  ...
  return (
    <>
      <rect 
        ...
        width={width} 
        height={height} 
        fill={environmentLandColor} 
      />
      <g className="environment" transform={`translate(${MARGIN}, ${MARGIN})`}>
        <rect x={0} y={0} width={contentWidth} height={contentHeight} fill="transparent" />

        {outlinePath && (
          <>
            {/* Opaque white pond underlay so land color does not bleed through */}
            <path
              d={outlinePath}
              fill="#ffffff"
              fillOpacity={1}
